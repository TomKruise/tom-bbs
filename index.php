...
<?php include 'inc/header.inc.php'?>
    <div id="hot" class="auto">
        ...
        <ul class="newlist">
            <!-- 20条 -->
            <li><a href="#">[库队]</a> <a href="#">bbs实战项目录制中...</a></li>
...
while($data_father=mysqli_fetch_assoc($result_father)){
    ...
    <div class="box auto">
        <div class="title">
            <a href="list_first.php?id=<?php echo $data_father['id']?>"><?php echo $data_father['module_name']?></a>
        </div>
        <div class="classList">
            <?php
            ...
            if(mysqli_num_rows($result_son)){
                while ($data_son=mysqli_fetch_assoc($result_son)){
                    ...
                    $query="select count(*) from bbs_content where module_id={$data_son['id']}";
                    $count_all=num($link,$query);
                    $html=<<<A
					<div class="childBox new">
						<h2><a href="list_second.php?id={$data_son['id']}">{$data_son['module_name']}</a> <span>(今日{$count_today})</span></h2>
						帖子：{$count_all}<br />
					</div>
A;
                    echo $html;
